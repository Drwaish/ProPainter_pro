...
def colorize_mask(pred_mask):
    save_mask = Image.fromarray(pred_mask.astype(np.uint8))
    save_mask = save_mask.convert(mode='L')
    save_mask.putpalette(_palette)
...
def video_type_input_tracking(SegTracker, input_video, io_args, video_name, frame_num=0):
    ...
    return  io_args['output_mask_dir'], io_args['output_masked_frame_dir']
...
#     return io_args['output_video'], f"{io_args['tracking_result_dir']}/{video_name}_pred_mask.zip"if result:
